...
# channels = [0] (Kırmızı), channels = [2] (Mavi)
...
# histogram eşitleme 
# karşıtlık arttırma (kontrast arttırma)
hist_equ = cv2.imread(os.path.join(".","data","hist_equ.jpg"),0)
plt.figure(), plt.imshow(hist_equ, cmap = "gray")

img_hist = cv2.calcHist([hist_equ], channels = [0], mask = None , histSize = [256], ranges = [0,256])
plt.figure(), plt.plot(img_hist)

eq_hist = cv2.equalizeHist(hist_equ)
